...
public abstract class AmazonS3Factory
{
    ...
    {
        AmazonS3 client = createAmazonS3(getCredentialsProvider(props),
                                         getClientConfiguration(props),
                                         getRequestMetricsCollector(props));
        if (uri.getHost() != null)
        {
            if (uri.getPort() != -1)
            {
     ...
     */
    protected abstract AmazonS3 createAmazonS3(AWSCredentialsProvider credentialsProvider,
    ...
    protected RequestMetricCollector getRequestMetricsCollector(Properties props)
    {
        ...
        if (props.containsKey(REQUEST_METRIC_COLLECTOR_CLASS))
        {
            try
            {
                requestMetricCollector = (RequestMetricCollector) Class.forName(props.getProperty(
                        REQUEST_METRIC_COLLECTOR_CLASS)).newInstance();
            }
            catch (Throwable t)
            {
                throw new IllegalArgumentException("Can't instantiate REQUEST_METRIC_COLLECTOR_CLASS " +
                                                   props.getProperty(REQUEST_METRIC_COLLECTOR_CLASS), t);
            }
        }
